...
- `speed_kph` : vitesse sur tapis roulant (km/h)
- `phase` : phase du protocole (REST, WARMUP, EXERCISE, etc.)

```{r}
# Importation du fichier Excel COSMED
# Étape 1 : Lire les noms de colonnes depuis la ligne 1 (colonnes J à BE)
...
# Étape 2 : Lire les données à partir de la ligne 4 (après métadonnées et unités)
...
# Étape 3 : Sélection et renommage des colonnes d'intérêt
data <- data_raw |>
  select(
    time = t,
    vo2 = VO2,
  ...
  mutate(
    # Conversion du temps : Excel stocke le temps comme POSIXct
    # On extrait heures, minutes, secondes → total en secondes depuis minuit
...
::: {.callout-note}
## Pourquoi ces étapes?
1. **Lignes 1-3** : Le fichier COSMED contient des métadonnées (participant, conditions) et une ligne d'unités qu'on doit ignorer
2. **Colonnes J-BE** : Les colonnes A-I contiennent des infos sur le participant; les données physiologiques commencent en colonne J
3. **Conversion du temps** : Excel stocke le temps comme un objet datetime qu'on convertit en secondes
4. **Sélection** : On garde uniquement les 8 variables essentielles parmi les 48 disponibles
:::
...
### 1.3 VO₂ en fonction de la charge externe {#sec-vo2-vitesse}

Comparons avec la relation VO₂ vs vitesse :

```{r}
ggplot(data, aes(x = speed_kph, y = vo2)) +
  geom_point(alpha = 0.5) +  # alpha = transparence (0-1)
  labs(
    title = "Relation VO₂ - Vitesse",
    x = "Vitesse (km/h)",
    y = "VO₂ (mL/min)"
  )
```
